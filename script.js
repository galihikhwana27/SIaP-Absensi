...
function navigateTo(pageId) {
    // Guard clause for admin trying to access teacher pages
    if (currentUser.role === 'Admin' && pageId === 'absensiGuruPage') {
        pageId = 'dashboardPage'; // Redirect admin to dashboard
    }
...
// Fitur Manajemen Guru (BARU)
function renderManajemenGuru() {
    const tableRows = Object.keys(mockUsers).filter(key => key !== 'admin').map((nip, index) => {
        const user = mockUsers[nip];
        return `
            <tr class="border-b hover:bg-gray-50">
                <td class="p-3 text-center">${index + 1}</td>
                ...
                <td class="p-3 text-center">
                    <button onclick="editGuru('${nip}')" class="text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                    <button onclick="hapusGuru('${nip}')" class="text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
                </td>
            </tr>
        `;
    }).join('');

    document.getElementById('manajemenGuruPage').innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-bold">Data Guru & Staf</h3>
                <button onclick="tambahGuru()" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600"><i class="fas fa-plus mr-2"></i> Tambah Pengguna</button>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead class="bg-gray-100">
...
function tambahGuru() {
    showModal('Tambah Pengguna Baru', `
        <form id="formGuru">
            <div class="mb-4">
                <label for="nipGuru" class="block text-sm font-medium text-gray-700">NIP</label>
                <input type="text" id="nipGuru" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" required>
            </div>
            <div class="mb-4">
                <label for="namaGuru" class="block text-sm font-medium text-gray-700">Nama Lengkap</label>
                <input type="text" id="namaGuru" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" required>
            </div>
            <div class="mb-4">
                <label for="roleGuru" class="block text-sm font-medium text-gray-700">Jabatan</label>
                <select id="roleGuru" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" required>
            ...
            <div class="mb-4">
                <label for="passwordGuru" class="block text-sm font-medium text-gray-700">Password</label>
                <input type="password" id="passwordGuru" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3" required>
            </div>
             <div id="waliKelasInputContainer" class="mb-4 hidden">
                <label for="waliKelasNama" class="block text-sm font-medium text-gray-700">Mengajar Kelas</label>
                <input type="text" id="waliKelasNama" placeholder="Contoh: IX A" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3">
            </div>
            <div class="text-right">
                <button type="button" onclick="closeModal()" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400 mr-2">Batal</button>
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">Simpan</button>
    ...
    document.getElementById('roleGuru').addEventListener('change', (e) => {
        document.getElementById('waliKelasInputContainer').classList.toggle('hidden', e.target.value !== 'Wali Kelas');
    });
    document.getElementById('formGuru').addEventListener('submit', simpanGuru);
}
...
function editGuru(nip) {
    ...
    if(user) {
        ...
        document.getElementById('nipGuru').readOnly = true;
        document.getElementById('namaGuru').value = user.name;
        document.getElementById('roleGuru').value = user.role;
        document.getElementById('passwordGuru').value = user.password;
        if(user.role === 'Wali Kelas') {
            document.getElementById('waliKelasInputContainer').classList.remove('hidden');
            document.getElementById('waliKelasNama').value = user.waliKelas;
        }
    }
}
...
function simpanGuru(e) {
    ...
    const name = document.getElementById('namaGuru').value;
    const role = document.getElementById('roleGuru').value;
    const password = document.getElementById('passwordGuru').value;

    const userData = { name, role, password };

    if (role === 'Wali Kelas') {
        userData.waliKelas = document.getElementById('waliKelasNama').value;
    }

    mockUsers[nip] = userData;
    
    showSuccessToast('Data pengguna berhasil disimpan.');
    renderManajemenGuru();
    closeModal();
}

function hapusGuru(nip) {
    if(confirm(`Yakin ingin menghapus pengguna dengan NIP ${nip}?`)) {
        delete mockUsers[nip];
